...
let
  target = import ../examples/target.nix {
    ...
    # Public-only fixture for environment.etc materialization; login uses a
    # separate key generated by the test driver.
    authorizedKeysFile = ./fixtures/authorized-key.pub;
    disk = "/dev/vdb";
    extraModules = [
      ({ lib, ... }: {
        ...
        services.openssh.settings = {
          ...
          AuthorizedKeysFile = lib.mkForce "/etc/ssh/authorized_keys/%u /etc/ssh/runtime-keys/%u";
        };
...
(nixos-lib.runTest {
  ...
  testScript = ''
    ...
    key_dir = tempfile.TemporaryDirectory(prefix="finix-test-ssh-")
    key = Path(key_dir.name) / "id_ed25519"
    subprocess.run(
        ["${pkgs.openssh}/bin/ssh-keygen", "-q", "-t", "ed25519", "-N", "", "-f", str(key)],
        check=True,
    )
    ...
    installer.succeed("install -d -m700 /root/.ssh")
    installer.copy_from_host(str(public_key), "/root/.ssh/authorized_keys")
    deployer.succeed("install -d -m700 /root/.ssh")
    deployer.copy_from_host(str(key), "/root/.ssh/install_key")
    deployer.copy_from_host(str(public_key), "/root/.ssh/install_key.pub")
    deployer.succeed("chmod 600 /root/.ssh/install_key")
    ...
    with subtest("install the real Finix closure through the public CLI"):
        deployer.succeed("""
          set -eu
          mkdir -p /tmp/extra/etc/ssh/runtime-keys /tmp/extra/home/operator/.ssh /tmp/extra/var/lib/credentials
          chmod 700 /tmp/extra/home/operator/.ssh /tmp/extra/var/lib/credentials
          install -m600 /root/.ssh/install_key.pub /tmp/extra/etc/ssh/runtime-keys/root
          install -m644 /root/.ssh/install_key.pub /tmp/extra/etc/ssh/runtime-keys/operator
    ...
    def check_installed():
        ...
        assert ssh("cat /etc/ssh/authorized_keys/root").stdout.strip() == Path("${./fixtures/authorized-key.pub}").read_text().strip()
